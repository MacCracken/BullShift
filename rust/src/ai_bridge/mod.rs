...
impl BearlyManaged {
    ...
    pub async fn add_provider(&mut self, mut provider: AIProvider) -> Result<Uuid, BullShiftError> {
        ...
        if !provider.api_key.is_empty() {
            self.security_manager
                .store_api_key(&provider.name, &provider.api_key)?;
    ...
    pub async fn test_provider_connection(
        ...
        match provider.provider_type {
            AIProviderType::OpenAI => self.test_openai_connection(provider, config).await,
            AIProviderType::Anthropic => self.test_anthropic_connection(provider, config).await,
            AIProviderType::Ollama => self.test_ollama_connection(provider, config).await,
            AIProviderType::LocalLLM => self.test_local_llm_connection(provider, config).await,
            AIProviderType::SecureYeoman => {
                self.test_secureyeoman_connection(provider, config).await
            }
            AIProviderType::Custom => self.test_custom_connection(provider, config).await,
        }
    }
    ...
    pub async fn generate_ai_strategy(
        &self,
        strategy: &TradingStrategy,
    ) -> Result<TradingStrategy, BullShiftError> {
        let provider = self
            .providers
            .get(&strategy.provider_id)
            .ok_or_else(|| BullShiftError::AiBridge("Provider not found".to_string()))?;
        ...
        if !response.success {
            return Err(BullShiftError::AiBridge(
                response
                    .error_message
                    .unwrap_or("AI request failed".to_string()),
            ));
        }
    ...
    async fn test_endpoint_connection(
        ...
        match self.client.get(url).send().await {
            ...
            Err(e) => Err(BullShiftError::AiBridge(format!(
    ...
    async fn test_openai_connection(
        ...
        self.test_endpoint_connection(&format!("{}/models", provider.api_endpoint), "OpenAI")
            .await
    }
    ...
    async fn test_anthropic_connection(
        ...
        self.test_endpoint_connection(
            &format!("{}/v1/messages", provider.api_endpoint),
            "Anthropic",
        )
        .await
    }
    ...
    async fn test_ollama_connection(
        ...
        self.test_endpoint_connection(&format!("{}/api/tags", provider.api_endpoint), "Ollama")
            .await
    }
    ...
    async fn test_local_llm_connection(
        ...
        self.test_endpoint_connection(&format!("{}/health", provider.api_endpoint), "Local LLM")
            .await
    }
    ...
    async fn test_custom_connection(
        ...
        self.test_endpoint_connection(&format!("{}/test", provider.api_endpoint), "Custom")
            .await
    }

    async fn test_secureyeoman_connection(
        &self,
        provider: &AIProvider,
        _config: &AIConfiguration,
    ) -> Result<bool, BullShiftError> {
        self.test_endpoint_connection(
            &format!("{}/api/v1/health", provider.api_endpoint),
            "SecureYeoman",
        )
        .await
    }
    ...
    async fn send_openai_request(
        ...
        let content = result["choices"][0]["message"]["content"]
            .as_str()
            .ok_or_else(|| {
                BullShiftError::AiBridge("Invalid OpenAI response format".to_string())
            })?;
        let tokens_used = result["usage"]["total_tokens"]
            .as_u64()
            .ok_or_else(|| BullShiftError::AiBridge("Invalid token usage data".to_string()))?
            as u32;
    ...
    async fn send_anthropic_request(
        ...
        let content = result["content"][0]["text"].as_str().ok_or_else(|| {
            BullShiftError::AiBridge("Invalid Anthropic response format".to_string())
        })?;
    ...
    async fn send_ollama_request(
        ...
        let content = result["response"].as_str().ok_or_else(|| {
            BullShiftError::AiBridge("Invalid Ollama response format".to_string())
        })?;
    ...
    async fn send_local_llm_request(
        &self,
        provider: &AIProvider,
        prompt: &str,
    ) -> Result<AIResponse, BullShiftError> {
        ...
        let content = result["generated_text"]
            ...
            .ok_or_else(|| {
                BullShiftError::AiBridge("Invalid Local LLM response format".to_string())
            })?;
        Ok(self.build_ai_response(provider, content, prompt.len() as u32, 0.0))
    }

    async fn send_secureyeoman_request(
        &self,
        provider: &AIProvider,
        prompt: &str,
    ) -> Result<AIResponse, BullShiftError> {
        ...
        let result = self
            .post_ai_request(provider, &url, &body, auth_headers)
            .await?;
        ...
        let content = result["choices"][0]["message"]["content"]
            ...
            .ok_or_else(|| {
                BullShiftError::AiBridge("Invalid SecureYeoman response format".to_string())
            })?;
    ...
    async fn send_custom_request(
        ...
        let content = result["text"]
            ...
            .ok_or_else(|| {
                BullShiftError::AiBridge("Invalid Custom provider response format".to_string())
            })?;
    ...
    fn encrypt_configuration(
        &self,
        config: &AIConfiguration,
    ) -> Result<AIConfiguration, BullShiftError> {
        // Encrypt API key
        let encrypted_api_key = self
            .security_manager
            .encrypt_sensitive_data(&config.api_key)?;
    ...
    pub fn update_provider_api_key(
        ...
        let provider = self
            ...
            .ok_or_else(|| BullShiftError::AiBridge("Provider not found".to_string()))?;
        self.security_manager
            .store_api_key(&provider.name, new_api_key)?;
    ...
    pub async fn get_provider_usage_stats(
        &self,
        provider_id: &Uuid,
    ) -> Result<UsageStats, BullShiftError> {
        let provider_responses: Vec<_> = self
            .responses
            .iter()
        ...
        let total_tokens = provider_responses
            .iter()
            .map(|r| r.tokens_used)
            .sum::<u32>();
...
mod tests {
    ...
    async fn test_api_key_encrypted_on_add() {
        ...
        assert!(
            stored.api_key.is_empty(),
            "plaintext key should be cleared from provider struct"
        );
    ...
    async fn test_api_key_update() {
        ...
        // Update the key
        bearly
            .update_provider_api_key(&provider_id, "sk-new-key-67890")
            .unwrap();
    ...
    fn test_security_manager_api_key_roundtrip() {
        ...
        sm.store_api_key("test_provider", "super-secret-key-123")
            .unwrap();
