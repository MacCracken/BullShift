...
class SecurityManager {
  ...
  static Future<void> storeCredentials({
    ...
    try {
      ...
      assert(() {
        debugPrint('Securely stored credentials for broker: $broker');
        return true;
      }());
    } catch (e) {
  ...
  static Future<Map<String, String>?> getCredentials(String broker) async {
    try {
      ...
      final encryptedApiKey =
          await _secureStorage.read(key: '${_keyPrefix}${broker}_api_key');
      final encryptedApiSecret =
          await _secureStorage.read(key: '${_keyPrefix}${broker}_api_secret');
  ...
  static Future<void> removeCredentials(String broker) async {
    try {
      ...
      assert(() {
        debugPrint('Removed credentials for broker: $broker');
        return true;
      }());
    } catch (e) {
  ...
  static Future<bool> validateCredentials(String broker) async {
    try {
      ...
      // Basic validation
      return apiKey.isNotEmpty && apiSecret.isNotEmpty && apiKey.length > 10;
    } catch (e) {
      return false;
    }
  }
  ...
  static Future<String> _decryptData(
      String encryptedData, String masterKey) async {
    try {
      // Derive 32-byte key from master key using SHA-256
      final keyBytes = sha256.convert(utf8.encode(masterKey)).bytes;
      final key = encrypt.Key(Uint8List.fromList(keyBytes));
